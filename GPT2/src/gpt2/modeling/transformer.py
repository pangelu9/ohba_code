...
class Transformer(nn.Module):
    ...
    def forward(self,
        ...
        x = self.dropout_embedding(x) # x shape: ([batch_size, seq_lenght-1, dims])
        ...
        x = self.ln_head(x)  # x shape: ([batch_size, seq_lenght-1, dims])
        x = self.token_embedding(x, transposed=True)
        # x shape: ([batch_size, seq_lenght-1, num_emb])
